...
class ListViewTest(TestCase):
    ...
    def test_can_save_a_POST_request_to_an_existing_list(self):
        ...
        response = self.client.post(
            ...
            {'text': 'A new hamid list item'}
        )

        self.assertEqual(Item.objects.count(), 1)
        new_item = Item.objects.first()
        self.assertEqual(new_item.text, 'A new hamid list item')
    ...
    @skip
    def test_for_duplicate_item_validation_error_end_up_on_lists_page(self):
        list1 = List.objects.create()
        item1 = Item.objects.create(list=list1, text='me awesome')
        response = self.client.post(f'/lists/{list1.id}/', 
            data={'text': 'me awesome'}
        )
